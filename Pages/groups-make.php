...
if($_POST) {
  ...
  echo 'few';
  foreach($_POST as $name) { echo $name.'few<br />'; }
  ...
  if(strlen($name) > 32 || strlen($name) < 4) { fatal_error('El nombre debe ternet entre 4 y 32 caracteres'); }
  ...
  if(!filter_var($_POST['imagen'], FILTER_VALIDATE_URL)) { fatal_error('La url de la im&aacute;n ingresada no es v&aacute;lida'); }
  ...
  if($_POST['idco'] && ctype_digit($_POST['idco'])) {
    ...
    if($ppp[1] != 0) { fatal_error('La comunidad fue eliminada'); }
    ...
    if($rank != 5) { fatal_error('No tienes permisos hijo de la re concha de tu hermana puto demierda'); }
    if($url != $pp[2]) { fatal_error('Qu&eacute; intentas!?'); }
    mysql_query('UPDATE `groups` SET `name` = \''.mysql_clean($name).'\', `desc` = \''.mysql_clean($_POST['descripcion']).'\', `avatar` = \''.mysql_clean($_POST['images']).'\', `cat` = \''.mysql_clean($_POST['categoria']).'\', `private` = \''.($_POST['privada'] ? 1 : 0).'\', `rankdefault` = \''.intval($_POST['rango_default']).'\' WHERE `id` = \''.$pp[0].'\' LIMIT 1') or fatal_error('Error en la query:'.mysql_error());
    fatal_error('La comunidad fue editada satisfactoriamente', 'LE VAMO A HACER LA FIESTA A TU HERMANA');
  }
}
if($_GET['id']) {
  ...
  //Miembro?
  if(!mysql_num_rows($few = mysql_query('SELECT `rank` FROM `groups_members` WHERE `group` = \''.$row['id'].'\' && `user` = \''.$logged['id'].'\''))) { fatal_error('No eres miembro de la comunidad'); }
  $rank = mysql_fetch_row($few);
  if($rank[0] != 5) { fatal_error('No tenes permisos para editar esta comunidad hijo de puta'); }
  $edit = true;
}
?>
<div id="main_content">
...
<li style="font-weight:bold;">Crear Comunidad</li><li class="last"></li>
</ul>
</div>
<div class="clear"></div>
...
Si tenes dudas sobre las comunidades visita <a href="<?=$config['url'];?>/ayuda/categoria/comunidades/">este enlace</a>.</div>
<br class="space">
...
<input onfocus="foco(this);" class="c_input" name="shortname" tabindex="2" onkeyup="comunidades.crear_shortname_key(this.value)" onblur="no_foco(this);comunidades.crear_shortname_check(this.value)" datatype="text"<?=($edit ? 'value="'.$row['url'].'" disabled="true"' : '');?> dataname="Nombre corto" style="width:254px;" type="text" />
...
<fieldset>
  ...
  <td><p class="descRadio">Toda persona que entra a dorantesm tiene la posibilidad de entrar y ver el contenido de tu comunidad.</p></td>
  </tr>
  </table>
</fieldset>
<fieldset>
  ...
  <td><p class="descRadio">Todo aquel que no este registrado en dorantesm no podr&aacute; ver el contenido de tu comunidad.</p></td>
  </tr>
  </table>
</fieldset>
</div>
