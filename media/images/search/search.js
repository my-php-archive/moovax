...
var search = {
	...
	home_change: function (select, a) {
		...
		switch (select) {
			case 'internet':
				$('form[name="search-box"]').attr('action', search_url + '/temas');
				$('#filterComunidades').show();
				$('.filter-adv').show();
				break;
			case 'posts':
				$('form[name="search-box"]').attr('action', search_url + '/posts?engine=moovax');
				$('#filterPosts').show();
				$('.filter-adv').show();
				break;
			case 'comunidades':
				$('form[name="search-box"]').attr('action', search_url +'/comunidades');
				$('#filterComunidades').show();
				$('.filter-adv').show();
				break;
			case 'empleos':
				$('form[name="search-box"]').attr('action', search_url + '/temas/'+$('input[name=paisempleo]').val());
				$('#filterComunidades').show();
				$('.filter-adv').show();
				break;
		}
	...
	filterSearch_show: function(){
		...
		if(es_visible){
			...
			$('.filterSearch').slideUp(500);
		}else{
			$('.filter-adv').removeClass('open');
			$('.filterSearch').slideDown(500);
		}
...
function login_ajax(){
	...
	$('#login_cargando').show();
	$.ajax({
		type: 'post', url: urls.home + '/login.php', cache: false, data: 'nick='+encodeURIComponent($('#nickname').val())+'&pass='+encodeURIComponent($('#password').val())+'&ajax=1',
		success: function (r) {
